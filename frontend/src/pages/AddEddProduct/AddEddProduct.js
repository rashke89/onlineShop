                        ...
                        <form>
                            <div className="form-floating mb-3">
                                <input type="text" name="title" className="form-control" id="title" placeholder="Title"
                                       value={product.hasOwnProperty("title") ? product.title : ""}
                            ...
                            <div className="form-floating mb-3">
                                <input type="text" className="form-control" id="description" name="description"
                                       maxLength="240" placeholder="Description"
                                       value={product.hasOwnProperty("title") ? product.description : ""}
                                       onChange={(event) => handleInputChange(event)}/>
                                <label htmlFor="description">Description</label>
                            </div>
                            <div className="form-floating mb-3">
                                <input type="number" className="form-control" id="price" name="price"
                                       placeholder="Price" value={product.hasOwnProperty("title") ? product.price : ""}
                                       onChange={(event) => handleInputChange(event)}/>
                            ...
                            <div className="form-floating mb-3">
                                <input type="text" className="form-control" id="imgUrl" name="imgUrl"
                                       ...
                                       value={product.hasOwnProperty("title") ? product.imgUrl : ""}
                                       onChange={(event) => handleInputChange(event)}/>
                                <label htmlFor="imgUrl">Image URL</label>
                            </div>
                            <button className="btn btn-secondary mt-3" onClick={(event) => {
                                onSubmit(product, product._id, event)
                            }}>{(isAddProduct) ? 'Add product' : "Save"}</button>
                            {!isFormValid ?
                                <p className="error animate__shakeX animate__animated animate__fast">All fields are
                                    required</p> : null}
                            {isApiError ?
                                <p className="error animate__shakeX animate__animated animate__fast">Api error. Try
                                    again.</p> : null}
